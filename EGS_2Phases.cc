...
namespace EGS_2Phases
{
  using namespace dealii;


//Pressure right hand side, pressure boundary values and saturation initial value classes
...
// Saturation Boundary Values
  ...
  // Saturation Boudary Values Hole
...
//Helper classes for solvers and preconditioners
  ...
  // construction of linear solvers and preconditioners.
  // The only difference is that the original variable name stokes_matrix
  // is replaced by another name darcy_matrix to match our problem.
...
//The TwoPhaseFlowProblem class
  ...
  // Buckley-Leverett problem [Buckley 1942])

  // This step uses one more ConstraintMatrix object called
  ...
  private:
    ...
    void assemble_saturation_rhs_boundary_term (const FEFaceValues<dim>             &saturation_fe_face_values,
                                                ...
                                                const std::vector<types::global_dof_index>     &local_dof_indices);
    void assemble_saturation_rhs_boundary_term_hole (const FEFaceValues<dim>             &saturation_fe_face_values,
                                                const FEFaceValues<dim>             &darcy_fe_face_values,
                                                const std::vector<types::global_dof_index>     &local_dof_indices);
    ...
    // model. We do this here as a member variable since
...
//TwoPhaseFlowProblem<dim>::assemble_darcy_system
  ...
  void TwoPhaseFlowProblem<dim>::assemble_darcy_system ()
  {
    ...
    // optimization to accelerate the assembly process.
    //
    ...
    // points. To this end, we can use the FEValues::get_function_values,a function
    ...
    for (; cell!=endc; ++cell, ++saturation_cell)
      {
             ...
             ++face_no)
        {
            ...
            //Changing the boundary of the hole
  ...
  void TwoPhaseFlowProblem<dim>::assemble_saturation_rhs ()
  {
      ...
      for (; cell!=endc; ++cell, ++darcy_cell)
      {
               ...
               ++face_no)
          {
              ...
              if (cell->at_boundary(face_no) && cell->face(face_no)->boundary_indicator() == 1)
              {
                  darcy_fe_face_values.reinit (darcy_cell, face_no);
                  saturation_fe_face_values.reinit (cell, face_no);
                  assemble_saturation_rhs_boundary_term_hole (saturation_fe_face_values,
                                                              darcy_fe_face_values,
                                                              local_dof_indices);
              }
          }
...
//TwoPhaseFlowProblem<dim>::assemble_saturation_rhs_boundary_term
  ...
  void
  TwoPhaseFlowProblem<dim>::
  assemble_saturation_rhs_boundary_term_hole (const FEFaceValues<dim>             &saturation_fe_face_values,
                                         const FEFaceValues<dim>             &darcy_fe_face_values,
                                         const std::vector<types::global_dof_index>     &local_dof_indices)
  {
    const unsigned int dofs_per_cell      = saturation_fe_face_values.dofs_per_cell;
    ...
    SaturationBoundaryValuesHole<dim> saturation_boundary_values;
    saturation_boundary_values
    .value_list (saturation_fe_face_values.get_quadrature_points(),
                 neighbor_saturation);
...
//TwoPhaseFlowProblem<dim>::get_extrapolated_saturation_range
  ...
  // saturation variable. This range is trivially bounded
...
//TwoPhaseFlowProblem<dim>::compute_viscosity
  ...
  // given cell. The major difference to that tutorial program is that the velocity here is
...
//    GridGenerator::hyper_cube (triangulation, 0, 1);
    ...
    GridIn<2> gridin;
        ...
        std::ifstream f("square_hole_2d.msh");
        gridin.read_msh(f);
